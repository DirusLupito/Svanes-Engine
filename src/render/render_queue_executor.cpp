...
void RenderQueueExecutor::Execute(RenderQueue &render_queue,
                                  std::optional<Rectangle2D> clip) const {
    SDL_Rect clip_rect{};
    if (clip) {
        clip_rect.x =
...
void RenderQueueExecutor::Execute(
    const RenderQueue::ClearCommand &command) const {
    // Set the draw color, then delegate to SDL_RenderClear to cleanse the
    // renderer.
    SetDrawColor(command.color);
    if (!SDL_RenderClear(renderer)) {
        throw std::runtime_error("Could not clear the renderer: " +
                                 std::string{SDL_GetError()});
    }
}

void RenderQueueExecutor::Execute(
    const RenderQueue::RectangleCommand &command) const {
    ...
    // Convert center-based rectangle coordinates to SDL's top-left-based
    ...
    if (command.rotation != 0.0F) {
        ...
        const auto corners =
            RectangleGeometry(command.destination, command.rotation).Corners();
        ...
        if (!SDL_RenderGeometry(renderer, nullptr, vertices, 4, indices, 6)) {
            throw std::runtime_error("Could not draw a rotated rectangle: " +
                                     std::string{SDL_GetError()});
        }
    } else if (!SDL_RenderFillRect(renderer, &destination)) {
        throw std::runtime_error("Could not draw a rectangle: " +
                                 std::string{SDL_GetError()});
    }
}

void RenderQueueExecutor::Execute(
    const RenderQueue::TriangleCommand &command) const {
    if (!SDL_SetRenderDrawBlendMode(renderer,
                                    ToSDLBlendMode(command.blend_mode))) {
        throw std::runtime_error("Could not set shape blending: " +
                                 std::string{SDL_GetError()});
    ...
    if (!SDL_RenderGeometry(renderer, nullptr, vertices, 3, nullptr, 0)) {
        throw std::runtime_error("Could not draw a triangle: " +
                                 std::string{SDL_GetError()});
    }
}

void RenderQueueExecutor::Execute(
    const RenderQueue::CircleCommand &command) const {
    if (!SDL_SetRenderDrawBlendMode(renderer,
                                    ToSDLBlendMode(command.blend_mode))) {
        throw std::runtime_error("Could not set shape blending: " +
                                 std::string{SDL_GetError()});
    ...
    // Need 3 indices per triangle, and we have segments many triangles in the
    // fan.
    std::array<std::int32_t, segments * 3> indices{};

    // The first vertex is the center of the circle, and the rest are the points
    // on the circumference.
    vertices[0] = {{command.destination.x, command.destination.y}, color, {}};

    // For each triangle, we need to calculate the angle and the corresponding
    // point on the circumference of the circle. Then we set up indices so that
    // each triple of indices[i*3], indices[i*3 + 1], indices[i*3 + 2] forms a
    // triangle in the fan. The first index is always 0 (the center), and the
    // other two are the current point and the next point (wrapping around to
    // the first point after the last).
    for (std::int32_t i = 0; i < segments; ++i) {

        // 2 pi * i / segments gives us the angle in radians for the current
        // segment.
        ...
        // Next point on the circumference, wrapping around to the first point
        // after the last.
...
void RenderQueueExecutor::Execute(
    const RenderQueue::ConvexPolygonCommand &command) const {
    if (!SDL_SetRenderDrawBlendMode(renderer,
                                    ToSDLBlendMode(command.blend_mode))) {
        throw std::runtime_error("Could not set shape blending: " +
                                 std::string{SDL_GetError()});
    ...
    if (points.size() >
        ...
        throw std::length_error(
            "Convex polygon has too many vertices to render.");
    }
    ...
    for (std::int32_t i = 1; i < static_cast<std::int32_t>(points.size()) - 1;
         ++i) {
        indices.push_back(0);
        indices.push_back(i);
    ...
    if (!SDL_RenderGeometry(renderer, nullptr, vertices.data(),
                            static_cast<std::int32_t>(vertices.size()),
                            indices.data(),
                            static_cast<std::int32_t>(indices.size()))) {
        throw std::runtime_error("Could not draw a convex polygon: " +
                                 std::string{SDL_GetError()});
    }
}

void RenderQueueExecutor::Execute(
    const RenderQueue::TextureCommand &command) const {
    ...
    const double rotation_degrees =
        static_cast<double>(command.rotation) * 180.0 / std::numbers::pi;

    if (!SDL_RenderTextureRotated(renderer, resolved_texture, source_pointer,
                                  &destination, rotation_degrees, nullptr,
                                  SDL_FLIP_NONE)) {
        throw std::runtime_error("Could not draw a texture: " +
                                 std::string{SDL_GetError()});
    }
}
...
void RenderQueueExecutor::SetDrawColor(Color color) const {
    if (!SDL_SetRenderDrawColor(renderer, color.red, color.green, color.blue,
                                color.alpha)) {
        throw std::runtime_error("Could not set the render draw color: " +
                                 std::string{SDL_GetError()});
    }
}
