 ...
 * providing functionality to load textures from files and create textures from
 * raw image data.
...
std::size_t TextureManager::GetExpectedPixelCount(const ImageData &image) {
    ...
    if (width > maximum_size / height ||
        width * height > maximum_size / channel_count) {
...
TextureHandle TextureManager::Store(TexturePointer texture) {
    ...
    // Instead, we use std::move to transfer ownership of the unique pointer
    // into the map. std::move doesn't actually move anything, it just casts the
    // unique pointer to something that can be moved from (if we ran
    // textures[handle.id] = texture; it would cause the compiler to error.
...
TextureHandle TextureManager::LoadTexture(std::string_view path) {
    ...
    if (texture == nullptr) {
        throw std::runtime_error("Could not load texture '" + path_string +
                                 "': " + SDL_GetError());
    }
...
TextureHandle TextureManager::CreateTexture(const ImageData &image) {
    ...
    if (image.rgba_pixels.size() != expected_pixel_count) {
        throw std::invalid_argument(
            "RGBA image data must contain exactly four bytes per pixel.");
    }
    ...
    TexturePointer texture{SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA32,
    ...
    // Pitch is like stride, but for 2D images. It is the number of bytes in a
    // row of pixel data including any padding. For RGBA images, each pixel is 4
    // bytes, so the pitch is width * 4.

    const std::int32_t pitch = image.width * 4;
    if (!SDL_UpdateTexture(texture.get(), nullptr, image.rgba_pixels.data(),
                           pitch)) {
...
TextureManager
internal::TextureManagerInternal::Create(SDL_Renderer *renderer) {
    if (renderer == nullptr) {
...
SDL_Texture *
internal::TextureManagerInternal::Resolve(const TextureManager &texture_manager,
    ...
    // .find() returns an iterator to the element if it exists, or
    // textures.end() if it does not.
    ...
    // texture is an iterator to a std::pair<const std::uint32_t,
    // TexturePointer> So we only care about the second element of the pair,
    // which is the unique pointer to the SDL_Texture.
