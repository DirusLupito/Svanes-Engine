...
/**
 * Validates that a rectangle's transform has finite values and positive
 * dimensions.
 *
 * @param rectangle The rectangle's geometry to validate.
 * @param transform The rectangle's transform to validate.
 *
 * @throws std::invalid_argument if the transform has non-finite values or
 * non-positive dimensions.
 */
static void ValidateRectangle(const Rectangle2D &rectangle,
    ...
    if (!std::isfinite(transform.x) || !std::isfinite(transform.y) ||
        ...
        !std::isfinite(transform.rotation) || rectangle.width <= 0.0F ||
        rectangle.height <= 0.0F) {
        throw std::invalid_argument("Collision rectangles require finite "
                                    "transforms and positive dimensions.");
    }
}

/**
 * Validates that a rectangle's transform has finite values and positive
 * dimensions, then returns the four corners in clockwise order starting from
 * the top-left corner.
 *
 * @param rectangle The rectangle's geometry to validate and extract corners
 * from.
 * @param transform The rectangle's transform to validate and extract corners
 * from.
 *
 * @return An array of four Vector2D objects representing the corners of the
 * rectangle in clockwise order starting from the top-left corner.
 */
static std::array<Vector2D, 4> RectangleVertices(const Rectangle2D &rectangle,
                                                 const Transform &transform) {
 ...
 *
 * @return std::pair<float, float> A pair containing the minimum and maximum
 * scalar projections.
 */
static std::pair<float, float>
ProjectPolygon(std::span<const Vector2D> vertices, Vector2D axis) {
    // How do we solve the problem of detecting contact between two convex
    // polygons? The Separating Axis Theorem (SAT) says that if a line exists
    // that separates the two polygons, they do not intersect. If no such line
    // exists, they do intersect. How do we find such a line? Well actually we
    // don't need to find one if we can prove that it does/does not exist.
    //
    // I recommend following along with this with either MS Paint or pen and
    // paper.
    //
    ...
    // How do we compute p? Well as we said, its the signed distance along the
    // axis to the projected point, which if we're looking at things from the
    // angle defined by the axis and the line from the origin to the vertex, is
    // the length of the adjacent side of the triangle. So we can use the cah
    // part of soh cah toa to get p = r cos(phi - theta) as r is the hypotenuse
    // and phi - theta is the angle between the hypotenuse and the adjacent
    // side. Simplifying we see:
    //
    //     p = r cos(phi - theta)
    ...
    //       = v dot n
    //
    // So it can be a negative distance if the projected point is in the
    ...
    //
    // Now as we explored previously, for those vertices v0 and v1, we can
    // compute their projections p0 and p1 onto the axis n. So we can then turn
    // our weighted average of the vertices into a weighted average of the
    // projections:
    //
    //     p(t) = (1-t)*p0 + t*p1,
    //
    // Interior points of a convex polygon are weighted averages of its
    // vertices. No point can project past the smallest or largest vertex
    // projection. Thus two numbers describe the whole projection.
    //
...
/**
 * Updates the collision information based on the projections of two convex
 * polygons or a convex polygon and a circle onto a given axis. If the
 * projections overlap, it calculates the penetration depth and updates the
 * collision normal.
 *
 * @param collision The current collision information to update.
 * @param axis The unit vector representing the axis onto which to project.
 * @param a_min The minimum scalar projection of the first polygon onto the
 * axis.
 * @param a_max The maximum scalar projection of the first polygon onto the
 * axis.
 * @param b_min The minimum scalar projection of the second polygon onto the
 * axis.
 * @param b_max The maximum scalar projection of the second polygon onto the
 * axis.
 *
 * @return true if the projections overlap (indicating a potential collision),
 * false otherwise.
 *
 * @throws std::invalid_argument if any of the projections are not finite,
 * or if the penetration depth is not finite.
 */
static bool UpdateCollision(Collision2D &collision, Vector2D axis, float a_min,
                            float a_max, float b_min, float b_max) {
    ...
    if (!std::isfinite(depth)) {
        throw std::invalid_argument(
            "Collision penetration depth must be finite.");
    }

    // To remove penetration, we only need the projected intervals to stop
    // penetrating on one axis. So let's choose the axis requiring the least
    // movement.
    //
...
/**
 * Tests convex polygons with at least three vertices ordered around each
 * boundary. Convexity is assumed, not checked.
 *
 * All vertices and edge lengths must be finite and edges nonzero.
 *
 * Vertices must be passed in consecutive order, e.g. clockwise or
 * counterclockwise around the polygon. The first and last vertices need not be
 * the same.
 *
 ...
 *
 * @throws std::invalid_argument for non-finite vertices or edges with
 * non-positive lengths.
 *
 * Reference: https://dyn4j.org/2010/01/sat/
 */
static std::optional<Collision2D>
DetectConvexCollision(std::span<const Vector2D> a,
                      std::span<const Vector2D> b) {
    Collision2D collision{{}, std::numeric_limits<float>::infinity()};

    // Hold any questions for a bit, and try to understand the next three
    // paragraphs.
    //
    // If two polygons overlap at some point, that means there is some
    // projection of that point onto any axis that lies within both polygons'
    // projected intervals.
    //
    // Therefore, a gap between their projected intervals proves they cannot
    // intersect. So if ANY, and I stress ANY, axis exists where the projections
    // do not overlap, then the polygons are separated.
    //
    // But overlap on just one axis doesn't really tell us much. If we could
    // check every possible axis, we could prove that the polygons intersect.
    // But there are infinitely many axes, so we can't check them all.
    //
    // So now we can start asking questions. Specifically, which axes do we need
    // to check?
    //
    // For convex polygons, SAT says the normals of BOTH polygons' edges
    // suffice. You can just accept that as a fact, or as I will attempt to
    ...
    //
    // The difference shape is also convex. To see why, choose two of its
    // points:
    //
    //     d0 = a0 - b0
    //     d1 = a1 - b1
    ...
    //
    // Imagine extending each edge of this difference polygon into an infinite
    // line. Each line has an interior side containing the polygon, and an
    // exterior side. A point is inside the polygon exactly when it is on the
    // interior side of every edge line, or on the boundary.
    //
    ...
    // greater than B's largest projection. Their projected intervals have a
    // gap!
    //
    ...
    //
    // If both choices identify a single vertex, their difference is just one
    // point. For this boundary to contain an entire edge, at least one choice
    // must allow movement along an edge of A or B without changing its
    // projection.
    //
    ...
    for (const auto vertices : {a, b}) {
        for (std::size_t i = 0; i < vertices.size(); ++i) {
            ...
            // Their dot product is dx*(-dy) + dy*dx = 0, so they are
            // perpendicular. Also, |n| = sqrt((-dy)^2 + dx^2) = |e|. Dividing
            // by the edge length therefore gives a unit normal.
            //
            // Without this division, the projection formula would give
            // v dot n = |v|*|n|*cos(phi-theta): all distances would be scaled
            // by |n|. Boolean overlap would still work, but comparing
            // penetration depths between axes from differently sized edges
    ...
    // At this point, we have checked every edge of both polygons. No gaps were
    // found.
    return collision;
}
 ...
 * The circle is projected onto the axis as an interval centered at its
 * projection with a length equal to its diameter. The polygon is projected onto
 * the axis as an interval defined by its minimum and maximum projections. If
 * the intervals overlap, the collision information is updated with the
 ...
 * or if the penetration depth is not finite.
 */
static bool TestCircleAxis(const Circle2D &circle,
                           std::span<const Vector2D> polygon, Vector2D axis,
    ...
    return UpdateCollision(collision, axis, projection - circle.radius,
                           projection + circle.radius, minimum, maximum);
}

/**
 * Detects collision between a circle and a convex polygon using the Separating
 * Axis Theorem. If a collision is found, it calculates the penetration depth
 * and normal of the collision.
 *
 * @param circle The circle to test against the polygon.
 * @param polygon The vertices of the convex polygon to test against the circle.
 *
 * @return std::optional<Collision2D> The collision information if a collision
 * is detected, or std::nullopt if no collision is detected.
 *
 * @throws std::invalid_argument if any of the projections are not finite,
 * or if the penetration depth is not finite.
 */
static std::optional<Collision2D>
DetectCirclePolygonCollision(const Circle2D &circle,
                             std::span<const Vector2D> polygon) {
    Collision2D collision{{}, std::numeric_limits<float>::infinity()};
    ...
    // infinitely many edges. So we cannot rely on the circle's edges to provide
    // axes to test.
    //
    // Instead, we can rely on the combination of the polygon's edge normals and
    // one more: an axis defined by the vector from the circle's center to the
    // closest point on the polygon.
    //
    // As for why, let's start with the earlier Minkowski difference argument I
    // explained for polygon-to-polygon collisions. Every point in a circle can
    // be represented as a 2D vector from the circle's center to a point on its
    // boundary.
    //
    // Let:
    ...
    //
    // Then what I am saying is that every point in the circle can be
    // represented as:
    //
    //     c + u, ||u|| <= r
    //
    // Every difference between our arbitrary polygon point p and a point in the
    // circle is then:
    //
    //     p - (c + u) = p - c - u
    //
    ...
    //
    // c is just some constant representing the circle's center, meaning that
    // this shape is just a translation of the shape given by
    //
    //    { p - u | p in polygon, ||u|| <= r }
    //
    // which is just the polygon minus a circle of radius r centered at the
    // origin. Notice that ||u|| <= r is satisfied by both +u and -u, so we can
    // also write this as
    //
    //    { p + u | p in polygon, ||u|| <= r }
    //
    // So our set of points in the Minkowski difference is just every single
    // point in the polygon expanded by a circle of radius r. You can visualize
    // this in your head by imagining taking your polygon and a big marker with
    // a circular tip whose radius is r, and tracing the outline of the polygon
    // with that marker, then filling in the interior. Edges will be expanded
    // outward by a distance of r, and corners will turn into circular arcs of
    // radius r. Call this our Minkowski sum of the polygon and a circle of
    // radius r.
    //
    // Now, getting back to the SAT argument, recall that the shapes intersect
    // if and only if the origin is contained in the Minkowski difference. That
    // means we need to check if 0 is in the set given by
    //
    //     { p + u | p in polygon, ||u|| <= r } - c
    //
    // which is equivalent to checking if c is in our Minkowski sum. How can we
    // check if c is in the Minkowski sum? Well, rather than check if it is,
    // suppose we assume it is not, i.e. the circle and polygon do not
    // intersect. Call: q - the closest point in the polygon to c
    //
    // Then define
    //
    //     d = ||c - q||,
    //     n = (c - q) / d
    //
    // Now n is a unit vector pointing from the closest point on the polygon to
    // the circle's center. We also know that d > r because we assumed the
    // circle and polygon do not intersect.
    //
    // Now we will show that a line through q perpendicular to n separates the
    // two shapes. Choose any other point p in the polygon. Convexity guarantees
    // that every point given by
    //
    //     q + t*(p - q), 0 <= t <= 1
    //
    // is also in the polygon. Now since q is the closest point to c, we know
    // that moving from p to q cannot bring us any closer to c. So we can write
    // the squared distance from c to any point on the line segment from p to q
    // as:
    //
    //     ||c - (q + t*(p - q))||^2 = ||(c - q) - t*(p - q)||^2
    //                               = ||c - q||^2 - 2*t*(c - q) dot (p - q) +
    //                               t^2*||p - q||^2
    //
    // If the term (c - q) dot (p - q) were positive, then for small t,
    // the squared distance would be less than ||c - q||^2, meaning we would
    // have found a point on the polygon closer to c than q, contradicting the
    // assumption that q is the closest point. Therefore, we must have (c - q)
    // dot (p - q) <= 0. If we then divide by d > 0, we have
    //
    //     n dot (p - q) <= 0
    //
    ...
    //     n dot p <= n dot q
    //
    // which holds for our arbitrary point p in the polygon. So the projection
    // of the polygon onto the axis defined by n attains its maximum at q dot n.
    // Projecting the circle onto the same axis gives an interval centered at c
    // dot n with a length of 2*r. Solving our definition of n for c tells us
    // that c = q + d*n, so
    //
    ...
    //     c dot n - r > q dot n
    //
    // So the circle's projection onto the axis defined by n lies entirely to
    // the right of the polygon's projection. Now how do we know if our axes
    // comprised of the polygon's edge normals and the axis pointing from the
    // circle's center to the closest vertex of the polygon are sufficient to
    // detect all collisions? Well we have a few cases:
    //
    // 1. The circle center is inside the polygon. Then on every axis, the
    // polygon's projection contains the circle's
    //   projection.
    // 2. The circle center is outside the polygon.
    //     a. The closest point on the polygon is a vertex. Then the axis from
    //     the circle's center to that vertex separates
    //        them according to the argument above.
    //     b. The closest point on the polygon is along an edge. Then we're
    //     already testing the edge's normal, and according
    //        to the argument above, that axis separates them.
    //
    // So no matter what, all we have to do is check the polygon's edge normals
    // and the axis from the circle's center to the closest point on the
    // polygon. If any of those axes separate the shapes, we know they do not
    // intersect. Otherwise, they must intersect.


    // Iterate over each vertex of the polygon to find the closest point to the
    // circle's center.
    for (std::size_t i = 0; i < polygon.size(); ++i) {
        ...
        if (!std::isfinite(distance)) {
            throw std::invalid_argument(
                "Circle-to-polygon distances must be finite.");
        }
        ...
        // While we're already iterating over the polygon's vertices, we can
        ...
        if (!std::isfinite(length) || length <= 0.0F) {
            throw std::invalid_argument(
                "Collision polygon edges require finite, positive lengths.");
        }

        if (!TestCircleAxis(circle, polygon, Vector2D{-edge.y, edge.x} / length,
                            collision)) {
            return std::nullopt;
        }
    }

    // At this point, no edge normals separated the shapes. All that's left is
    // to check the axis from the circle's center to the closest point on the
    // polygon. If that axis separates them, we know they do not intersect.
    if (closest_distance > 0.0F &&
        !TestCircleAxis(circle, polygon, closest / closest_distance,
                        collision)) {
        return std::nullopt;
    }
 ...
 *
 * @throws std::invalid_argument for non-finite transforms, nonpositive
 * dimensions, or rectangle edges that cannot be represented with a finite,
 * positive length.
 */
static std::optional<Collision2D>
DetectCollision(const Rectangle2D &a, const Transform &transform_a,
 ...
 * the first axis encountered is the one returned.
 *
 * @throws std::invalid_argument for triangle edges that cannot be represented
 * with a finite, positive length.
 */
static std::optional<Collision2D>
DetectCollision(const Triangle2D &a, const Transform &transform_a,
...
/**
 * Detects contact between a triangle and a rectangle. Describes the nature or
 * lack thereof of the contact as both a normal and a penetration depth.
 *
 ...
 *
 * @throws std::invalid_argument for non-finite transforms, nonpositive
 * dimensions, or triangle edges that cannot be represented with a finite,
 * positive length.
 */
static std::optional<Collision2D>
DetectCollision(const Triangle2D &a, const Transform &transform_a,
...
/**
 * Detects contact between a rectangle and a triangle. Describes the nature or
 * lack thereof of the contact as both a normal and a penetration depth.
 *
 ...
 * the first axis encountered is the one returned.
 *
 * @throws std::invalid_argument for non-finite transforms or non-positive
 * radii.
 */
static std::optional<Collision2D>
DetectCollision(const Circle2D &a, const Transform &transform_a,
    ...
    if (!std::isfinite(distance) || !std::isfinite(radii)) {
        throw std::invalid_argument(
            "Circle collision distances must be finite.");
    }
    ...
    // As for the normal, we can just use the vector from the center of b to the
    // center of a, normalized. And the penetration depth is the sum of the
    // radii minus the distance between the centers.

    return Collision2D{distance > 0.0F ? offset / distance
                                       : Vector2D{1.0F, 0.0F},
                       radii - distance};
}

/**
 * Detects contact between a circle and a rectangle. Describes the nature or
 * lack thereof of the contact as both a normal and a penetration depth.
 *
 ...
 *
 * @throws std::invalid_argument for non-finite transforms, nonpositive
 * dimensions, or non-positive radii.
 */
static std::optional<Collision2D>
DetectCollision(const Circle2D &a, const Transform &transform_a,
                const Rectangle2D &b, const Transform &transform_b) {
...
/**
 * Detects contact between a rectangle and a circle. Describes the nature or
 * lack thereof of the contact as both a normal and a penetration depth.
 *
 * @param a The rectangle's geometry.
 * @param transform_a The rectangle's transform.
...
/**
 * Detects contact between a circle and a triangle. Describes the nature or lack
 * thereof of the contact as both a normal and a penetration depth.
 *
 * @param a The circle's geometry.
 * @param transform_a The circle's transform.
 * @param b The triangle's vertices.
...
/**
 * Detects contact between a triangle and a circle. Describes the nature or lack
 * thereof of the contact as both a normal and a penetration depth.
 *
 * @param a The triangle's vertices.
...
/**
 * Detects all collisions between two primitive 2D shapes, which may be of
 * different types (e.g., circle, rectangle, triangle). The function uses
 * std::visit to handle the different shape types and calls the appropriate
 * collision detection function for for each pair of shapes. If a collision is
 * detected, it is added to the collisions vector.
 *
 ...
 */
static std::optional<Collision2D>
DetectCollision(const ConvexPolygon2D &a, const Transform &transform_a,
                const Rectangle2D &b, const Transform &transform_b) {
...
/**
 * Detects all collisions between a circle and a convex polygon. The function
 * transforms both shapes into world space using their respective transforms and
 * then calls the DetectCirclePolygonCollision function to check for collisions.
 *
...
/**
 * Detects all collisions between a rectangle and a convex polygon. The function
 * transforms both shapes into world space using their respective transforms and
 * then calls the DetectConvexCollision function to check for collisions.
 *
...
/**
 * Detects all collisions between a triangle and a convex polygon. The function
 * transforms both shapes into world space using their respective transforms and
 * then calls the DetectConvexCollision function to check for collisions.
 *
...
/**
 * Detects all collisions between a convex polygon and a circle. The function
 * transforms both shapes into world space using their respective transforms and
 * then calls the DetectCirclePolygonCollision function to check for collisions.
 *
...
static void AppendCollisions(const Primitive2D &a, const Transform &transform_a,
                             const Primitive2D &b, const Transform &transform_b,
                             std::vector<Collision2D> &collisions) {
    const auto collision = std::visit(
        [&](const auto &shape_a, const auto &shape_b) {
...
/**
 * Detects all collisions between a composite 2D shape, which may contain
 * multiple parts, and a primitive 2D shape. The function iterates through each
 * part of the composite shape and checks for collisions with the primitive
 * shape, applying the appropriate transforms to each part.
 *
 ...
 * @param collisions A vector to store the detected collisions.
 */
static void AppendCollisions(const CompositeShape2D &a,
                             const Transform &transform_a, const Primitive2D &b,
                             const Transform &transform_b,
                             std::vector<Collision2D> &collisions) {
    for (const GeometryPart2D &part : a.parts) {
        AppendCollisions(part.shape,
                         ComposeTransforms(transform_a, part.transform), b,
                         transform_b, collisions);
    }
}

/**
 * Detects all collisions between a primitive 2D shape and a composite 2D shape,
 * which may contain multiple parts. The function iterates through each part of
 * the composite shape and checks for collisions with the primitive shape,
 ...
 */
static void AppendCollisions(const Primitive2D &a, const Transform &transform_a,
                             const CompositeShape2D &b,
                             const Transform &transform_b,
                             std::vector<Collision2D> &collisions) {
    for (const GeometryPart2D &part : b.parts) {
        AppendCollisions(a, transform_a, part.shape,
                         ComposeTransforms(transform_b, part.transform),
                         collisions);
    }
}

/**
 * Detects all collisions between two composite 2D shapes, which may contain
 * multiple parts. The function iterates through each part of the first
 * composite shape and checks for collisions with the second composite shape,
 * applying the appropriate transforms to each part.
 *
...
std::vector<Collision2D> DetectCollisions(const Geometry2D &a,
    ...
    // we need to delegate to the appropriate overload of AppendCollisions based
    // on the types of a and b, (either primitive or composite).
    std::visit(
        [&](const auto &shape_a, const auto &shape_b) {
            AppendCollisions(shape_a, transform_a, shape_b, transform_b,
                             collisions);
        },
        a, b);
    return collisions;
...
DetectTileMapCollisions(const Geometry2D &geometry, const Transform &transform,
    ...
    if (tile_map.collision_cache_built) {
        for (const Rectangle2D &tile_geometry :
             tile_map.collision_rectangles) {
            std::vector<Collision2D> tile_collisions = DetectCollisions(
                geometry, transform, Geometry2D{tile_geometry},
                tile_map_transform);
            ...
            for (std::uint32_t y = 0; y < height; ++y) {
                for (std::uint32_t x = 0; x < width; ++x) {
                    visited[static_cast<std::size_t>(row + y) *
                                tile_map.columns + column + x] = true;
                }
            }
