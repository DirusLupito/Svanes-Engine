 ...
 * This class is responsible for handling the internal logic of input
 * management, including translating SDL events to the engine's input system.
...
public:
     ...
     * state and reset the current state for keys and mouse buttons.
     *
     * In total, this function must:
     *
     * - Update the previous state of keys and mouse buttons to match the
     * current state.
     *
     * - Reset the current state of keys and mouse buttons to false (not
     * pressed).
     *
     ...
     * to the InputManager's state that need to be reset or updated at the start
     * of each frame.
     *
     ...
     * This function checks if text input has been requested or stopped and
     * updates the InputManager's state accordingly.
...
private:
     ...
     * @return An optional Key value. If the scancode does not correspond to a
     * known key, std::nullopt is returned.
     */
     ...
     *   If the button code does not correspond to a known mouse button,
