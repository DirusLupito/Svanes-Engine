...
// Reference width for scaling and centering the game world on different output
// sizes.
constexpr std::int32_t kDesignWidth = 1920;

// Reference height for scaling and centering the game world on different output
// sizes.
 ...
 *
 * @throws std::invalid_argument if the factor is not finite or is less than or
 * equal to zero.
 */
static void ValidateFactor(float factor) {
    if (!std::isfinite(factor) || factor <= 0.0F) {
        throw std::invalid_argument(
            "Camera factor must be finite and greater than zero.");
    }
}
 ...
 *
 * @return true if the rectangle is outside the bounds of the rendering output,
 * false otherwise.
 */
static bool IsOutsideOutput(const Rectangle2D &bounds,
                            std::int32_t output_width,
                            std::int32_t output_height) {
    return bounds.x + bounds.width * 0.5F <= 0.0F ||
           bounds.y + bounds.height * 0.5F <= 0.0F ||
 ...
 * @param output_height The height of the rendering output.
 *
 * @return true if the circle is outside the bounds of the rendering output,
 * false otherwise.
 */
static bool IsOutsideOutput(const Circle2D &circle, std::int32_t output_width,
                            std::int32_t output_height) {
    // Find the closest point on the rectangle defined by the output dimensions
    // to the center of the circle.
    const float closest_x =
        std::clamp(circle.x, 0.0F, static_cast<float>(output_width));
    const float closest_y =
        std::clamp(circle.y, 0.0F, static_cast<float>(output_height));

    // If the distance from the circle's center to this closest point is greater
    // than or equal to the radius, then the circle is outside the bounds of the
    // output.
    return std::hypot(circle.x - closest_x, circle.y - closest_y) >=
           circle.radius;
}
...
std::int32_t Camera2D::OutputWidth() const { return output_width; }

std::int32_t Camera2D::OutputHeight() const { return output_height; }

float Camera2D::Scale() const {
    if (scale_mode == ScaleMode::Constant || output_width <= 0 ||
        output_height <= 0) {
        return 1.0F;
    }

    const float width_ratio =
        static_cast<float>(output_width) / static_cast<float>(kDesignWidth);
    const float height_ratio =
        static_cast<float>(output_height) / static_cast<float>(kDesignHeight);
...
Rectangle2D Camera2D::Viewport() const {
    ...
    if (scale_mode == ScaleMode::Proportional && output_width > 0 &&
        output_height > 0) {
...
void Camera2D::SetZoomAt(float new_zoom, Vector2D screen_position) {
    ...
    // We want to keep the world coordinates of the point at (screen_x,
    // screen_y) the same before and after the zoom change. ScreenToWorld of the
    // same point before and after the zoom change should yield the same world
    // coordinates.

    // So we need to update the center of the camera (x, y)
    // to ensure that the world coordinates of the point at (screen_x, screen_y)
    // remain unchanged.

    // Let (screen_x, screen_y) be the point in screen coordinates that we want
    // to keep anchored. Let (world_x, world_y) be the corresponding point in
    // world coordinates before the zoom change. Let (new_world_x, new_world_y)
    // be the corresponding point in world coordinates after the zoom change. We
    // want (world_x, world_y) to be equal to (new_world_x, new_world_y).

    // Rendering computes screen_x = (world_x - x) * zoom * scale + viewport.x,
    // and likewise for y. ScreenToWorld reverses those operations.
    // In constant mode, scale is 1. The viewport center is the output center in
    // both modes.
    //
    ...
    const Rectangle2D anchor =
        ScreenToWorld({screen_position.x, screen_position.y, 0.0F, 0.0F});
    zoom = new_zoom;
...
std::optional<Circle2D>
Camera2D::PrepareForRendering(const Transform &transform,
...
Camera2D::PrepareForRendering(const Transform &transform,
    ...
    if (IsOutsideOutput(TriangleGeometry(destination).Bounds(), output_width,
                        output_height)) {
        return std::nullopt;
    }
...
Camera2D::PrepareForRendering(const Transform &transform,
    ...
    // If the rectangle is infinitely small, or the output is infinitely small,
    // we can skip rendering it.
    if (output_width <= 0 || output_height <= 0 || rectangle.width <= 0.0F ||
        rectangle.height <= 0.0F) {
        return std::nullopt;
    }

    Rectangle2D destination =
        WorldToScreen(TransformRectangle(rectangle, transform));


    // Next, we need to check if the rectangle is within the bounds of the
    // rendering output.

    const Rectangle2D bounds =
        RectangleGeometry(destination, transform.rotation).Bounds();
...
Camera2D::PrepareForRendering(const Transform &transform,
    ...
    auto destination =
        TransformConvexPolygon(polygon, screen_transform, zoom * scale);
