...
EvaluateAttractors(const Registry &world,
    ...
    // iterate over all attractors and apply their acceleration to all other
    // entities. While right now this is about as efficient as the other way, if
    // we ever add a spatial partitioning system, this will be far more
    // efficient, as rather than have every entity query, say, all entities in
    // the surrounding 9 grid cells, we can instead have each attractor query
    // the surrounding 9 grid cells and apply its acceleration to all entities
    // in those cells.
    ...
    world.ForEach<Transform, PointAttractor2D>(
        [&](Entity source_entity, const Transform &source,
            ...
            for (const PhysicsTimeStep &step : entity_steps) {
                ...
                const Vector2D offset_to_source =
                    Vector2D{source.x, source.y} - Vector2D{target.x, target.y};
