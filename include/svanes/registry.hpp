...
/**
 * The entity component system registry. This class manages entities and their
 * components.
 *
 * Read through entity_components.md for a more detailed explanation of how
 * components work with entities.
 *
 * It allows for creation and destruction of entities, as well as adding,
 * removing, and querying components associated with those entities.
 *
 */
...
public:
    ...
    /**
     * Adds a component of type T to the specified entity. If the entity already
     * has a component of that type, it will be replaced.
     *
     * Gets a reference to where the component will live in the registry, writes
     * the new component to that location and passes in the provided arguments
     * to the component's constructor, and returns a reference to the newly
     * added component
     *
     * Example usage:
     *
     ...
     *
     * If the entity does not have a component of that type, this function is a
     * no-op.
     *
    ...
    /**
     * Gets a reference to the component of the given type associated with the
     * given entity.
     *
    ...
    template <typename T> T &GetComponent(Entity entity) {
        return std::any_cast<T &>(
            components.at(entity).at(std::type_index{typeid(T)}));
    }
...
private:
    ...
    // A 2D map of all components in the registry, first indexed by entity and
    // then component type.
