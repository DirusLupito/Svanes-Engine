...
/**
 * Manages textures for rendering, providing functionality to load textures from
 * files and create textures from raw image data. The TextureManager class is
 * responsible for managing the lifetime of SDL_Texture objects. It provides
 * methods to load textures from files and create textures from raw image data,
 ...
 * allowing the internal class to access private members such as the textures
...
class TextureManager final {
  public:
     ...
     * Throws an exception if the path is empty or if the texture cannot be
     * loaded.
     ...
     * Throws an exception if the image data is invalid or if the texture cannot
     * be created.
     ...
     * @throws std::runtime_error if the texture cannot be created or if the
     * pixel data cannot be uploaded.
     */
  ...
  private:
     ...
     * that needs to be freed is the SDL_Texture itself.
     *
     ...
     * Whereas here by passing in a struct which has a call operator, the
     * unique_ptr code will already try to call the operator() on the deleter.
    ...
    // Unique pointers take two template parameters: the type of the object
    // being managed and the deleter type.
    ...
    /**
     * Calculates the expected number of pixels in an image based on its width
     * and height. Throws an exception if the width or height is non-positive or
     * if the calculated pixel count exceeds the maximum size of std::size_t.
     * @param image The image data for which to calculate the expected pixel
     * count.
     * @return The expected number of pixels in the image.
     * @throws std::invalid_argument if the image dimensions are invalid or too
     * large.
     */
    ...
    // A hash map. Keys are texture IDs (uint32_t), and values are unique
